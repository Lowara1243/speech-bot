...
from src.speech_bot.core.config import BOT_TOKEN, RESET_SCHEDULE
from src.speech_bot.core.database import Database
from src.speech_bot.handlers.user_commands import start_router
from src.speech_bot.handlers.handle_audio_voice import audio_router
...
async def main():
    ...
    if not cron_string:
        ...
        error_message = (
            f"Неверное значение для RESET_SCHEDULE: '{RESET_SCHEDULE}'. Допустимые значения: {valid_options}"
        )
    ...
    async def reset_task():
        ...
        await Database.reset_all_remaining_time()
